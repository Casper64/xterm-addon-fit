"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var chai_1 = require("chai");
var TestUtils_1 = require("../../../out-test/api/TestUtils");
var APP = 'http://127.0.0.1:3001/test';
var browser;
var page;
var width = 1024;
var height = 768;
describe('FitAddon', function () {
    before(function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4, (0, TestUtils_1.launchBrowser)()];
                    case 1:
                        browser = _a.sent();
                        return [4, browser.newContext()];
                    case 2: return [4, (_a.sent()).newPage()];
                    case 3:
                        page = _a.sent();
                        return [4, page.setViewportSize({ width: width, height: height })];
                    case 4:
                        _a.sent();
                        return [4, page.goto(APP)];
                    case 5:
                        _a.sent();
                        return [2];
                }
            });
        });
    });
    beforeEach(function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4, page.evaluate("document.querySelector('#terminal-container').style.display=''")];
                    case 1:
                        _a.sent();
                        return [4, (0, TestUtils_1.openTerminal)(page)];
                    case 2:
                        _a.sent();
                        return [2];
                }
            });
        });
    });
    after(function () { return __awaiter(void 0, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, browser.close()];
                case 1:
                    _a.sent();
                    return [2];
            }
        });
    }); });
    afterEach(function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4, page.evaluate("window.term.dispose()")];
                    case 1:
                        _a.sent();
                        return [2];
                }
            });
        });
    });
    it('no terminal', function () {
        return __awaiter(this, void 0, void 0, function () {
            var _a, _b;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0: return [4, page.evaluate("window.fit = new FitAddon();")];
                    case 1:
                        _c.sent();
                        _b = (_a = chai_1.assert).equal;
                        return [4, page.evaluate("window.fit.proposeDimensions()")];
                    case 2:
                        _b.apply(_a, [_c.sent(), undefined]);
                        return [2];
                }
            });
        });
    });
    describe('proposeDimensions', function () {
        afterEach(function () { return __awaiter(void 0, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4, unloadFit()];
                    case 1: return [2, _a.sent()];
                }
            });
        }); });
        it('default', function () {
            return __awaiter(this, void 0, void 0, function () {
                var dimensions;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4, loadFit()];
                        case 1:
                            _a.sent();
                            return [4, page.evaluate("window.fit.proposeDimensions()")];
                        case 2:
                            dimensions = _a.sent();
                            chai_1.assert.equal(dimensions.cols, 87);
                            chai_1.assert.isAbove(dimensions.rows, 24);
                            chai_1.assert.isBelow(dimensions.rows, 29);
                            return [2];
                    }
                });
            });
        });
        it('width', function () {
            return __awaiter(this, void 0, void 0, function () {
                var dimensions;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4, loadFit(1008)];
                        case 1:
                            _a.sent();
                            return [4, page.evaluate("window.fit.proposeDimensions()")];
                        case 2:
                            dimensions = _a.sent();
                            chai_1.assert.equal(dimensions.cols, 110);
                            chai_1.assert.isAbove(dimensions.rows, 24);
                            chai_1.assert.isBelow(dimensions.rows, 29);
                            return [2];
                    }
                });
            });
        });
        it('small', function () {
            return __awaiter(this, void 0, void 0, function () {
                var _a, _b;
                return __generator(this, function (_c) {
                    switch (_c.label) {
                        case 0: return [4, loadFit(1, 1)];
                        case 1:
                            _c.sent();
                            _b = (_a = chai_1.assert).deepEqual;
                            return [4, page.evaluate("window.fit.proposeDimensions()")];
                        case 2:
                            _b.apply(_a, [_c.sent(), {
                                    cols: 2,
                                    rows: 1
                                }]);
                            return [2];
                    }
                });
            });
        });
        it('hidden', function () {
            return __awaiter(this, void 0, void 0, function () {
                var _a, _b;
                return __generator(this, function (_c) {
                    switch (_c.label) {
                        case 0: return [4, page.evaluate("window.term.dispose()")];
                        case 1:
                            _c.sent();
                            return [4, page.evaluate("document.querySelector('#terminal-container').style.display='none'")];
                        case 2:
                            _c.sent();
                            return [4, page.evaluate("window.term = new Terminal()")];
                        case 3:
                            _c.sent();
                            return [4, page.evaluate("window.term.open(document.querySelector('#terminal-container'))")];
                        case 4:
                            _c.sent();
                            return [4, loadFit()];
                        case 5:
                            _c.sent();
                            _b = (_a = chai_1.assert).equal;
                            return [4, page.evaluate("window.fit.proposeDimensions()")];
                        case 6:
                            _b.apply(_a, [_c.sent(), undefined]);
                            return [2];
                    }
                });
            });
        });
    });
    describe('fit', function () {
        afterEach(function () { return __awaiter(void 0, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4, unloadFit()];
                    case 1: return [2, _a.sent()];
                }
            });
        }); });
        it('default', function () {
            return __awaiter(this, void 0, void 0, function () {
                var cols, rows;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4, loadFit()];
                        case 1:
                            _a.sent();
                            return [4, page.evaluate("window.fit.fit()")];
                        case 2:
                            _a.sent();
                            return [4, page.evaluate("window.term.cols")];
                        case 3:
                            cols = _a.sent();
                            return [4, page.evaluate("window.term.rows")];
                        case 4:
                            rows = _a.sent();
                            chai_1.assert.equal(cols, 87);
                            chai_1.assert.isAbove(rows, 24);
                            chai_1.assert.isBelow(rows, 29);
                            return [2];
                    }
                });
            });
        });
        it('width', function () {
            return __awaiter(this, void 0, void 0, function () {
                var cols, rows;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4, loadFit(1008)];
                        case 1:
                            _a.sent();
                            return [4, page.evaluate("window.fit.fit()")];
                        case 2:
                            _a.sent();
                            return [4, page.evaluate("window.term.cols")];
                        case 3:
                            cols = _a.sent();
                            return [4, page.evaluate("window.term.rows")];
                        case 4:
                            rows = _a.sent();
                            chai_1.assert.equal(cols, 110);
                            chai_1.assert.isAbove(rows, 24);
                            chai_1.assert.isBelow(rows, 29);
                            return [2];
                    }
                });
            });
        });
        it('small', function () {
            return __awaiter(this, void 0, void 0, function () {
                var _a, _b, _c, _d;
                return __generator(this, function (_e) {
                    switch (_e.label) {
                        case 0: return [4, loadFit(1, 1)];
                        case 1:
                            _e.sent();
                            return [4, page.evaluate("window.fit.fit()")];
                        case 2:
                            _e.sent();
                            _b = (_a = chai_1.assert).equal;
                            return [4, page.evaluate("window.term.cols")];
                        case 3:
                            _b.apply(_a, [_e.sent(), 2]);
                            _d = (_c = chai_1.assert).equal;
                            return [4, page.evaluate("window.term.rows")];
                        case 4:
                            _d.apply(_c, [_e.sent(), 1]);
                            return [2];
                    }
                });
            });
        });
    });
});
function loadFit(width, height) {
    if (width === void 0) { width = 800; }
    if (height === void 0) { height = 450; }
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, page.evaluate("\n    window.fit = new FitAddon();\n    window.term.loadAddon(window.fit);\n    document.querySelector('#terminal-container').style.width='".concat(width, "px';\n    document.querySelector('#terminal-container').style.height='").concat(height, "px';\n  "))];
                case 1:
                    _a.sent();
                    return [2];
            }
        });
    });
}
function unloadFit() {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, page.evaluate("window.fit.dispose();")];
                case 1:
                    _a.sent();
                    return [2];
            }
        });
    });
}
//# sourceMappingURL=FitAddon.api.js.map